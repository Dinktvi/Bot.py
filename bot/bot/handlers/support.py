...
async def assistant_message(message: Message):
    ...
    answer, elapsed, images = await asyncio.to_thread(llm.ask, user_text, lang, history)
    if answer is None:
    ...
    for img in images:
        try:
            ...
            print("[assistant] send image failed:", e)
...
async def support_message(message: Message):
    ...
    answer, elapsed, images = await asyncio.to_thread(llm.ask, message.text or "", lang, history)
    if answer is None:
        await thinking.delete()
        await message.answer(_("assistant_no_key", lang), reply_markup=main_menu_kb(lang))
        return
    ...
    for img in images:
        try:
            ...
            print("[support] send image failed:", e)
